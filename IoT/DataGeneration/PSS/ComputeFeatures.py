...
def computeEmbeddings(toDo):
	...
	for x in toDo:
		...
		try:		
			...
			CallGraphSpectrum = CallGraphSpectrum/np.linalg.norm(CallGraphSpectrum)
			...
			embedding =  [CallGraphSpectrum, len(CallGraphSpectrum), edgesCFG, len(edgesCFG), elapsedPreprocessing]
...
if __name__ == '__main__':
	goodware = []
	for path in Path('../DATA/jsons').rglob('*'):
		pathJson = str(path)
		goodware += [pathJson]
