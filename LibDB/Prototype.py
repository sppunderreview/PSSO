    ...
    for XYZ in range(len(TR)):
        ...
        for (DS2, idS2, pairsF) in finalCandidates:
            ...
            if similarityScore > bestScore:
                ...
                mostSimilars = [(DS2, idS2)]
            elif similarityScore > 0 and similarityScore == bestScore:
                mostSimilars += [(DS2, idS2)]

        elapsed = time.time() - start

        if not(DS in closestD):
            closestD[DS] = {}

        if len(mostSimilars) == 0:
            print("RIEN ICI", DS,idS, len(finalCandidates))
            RR2 = deepcopy(RR)
            if (DS,idS) in RR2:
                RR2.remove((DS,idS))
            mostSimilar = choice(RR2)
        else:
            mostSimilar = choice(mostSimilars)

        closestD[DS][idS] = (mostSimilar[0], mostSimilar[1], elapsed)

    with open(outputFile, "wb") as f:
        pickle.dump(closestD, f)
