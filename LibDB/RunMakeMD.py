...
def minRun(O, folder, nameXP):
    ...
    closestDO0 = computeClosests(R_O0, R_O1)
    for (idS,path,compilerOption, name, pathJson) in O0:
        MD["->"][idS] = {}
        for (idS2,path2,compilerOption2,name2, pathJson2) in O1:
            if closestDO0[nameDS][idS][1] == idS2:
                d = 0
            else:
                d = 1
            MD["->"][idS][idS2] = (name,name2,compilerOption,compilerOption2,d, closestDO0[nameDS][idS][2])

    closestDO1 = computeClosests(R_O1, R_O0)
    for (idS,path,compilerOption,name, pathJson) in O1:
        MD["<-"][idS] = {}
        for (idS2,path2,compilerOption2,name2, pathJson2) in O0:
            if closestDO1[nameDS][idS][1] == idS2:
                d = 0
            else:
                d = 1
            MD["<-"][idS][idS2] = (name,name2,compilerOption,compilerOption2,d, closestDO1[nameDS][idS][2])

    OAll = O0 + O1

    closestDALL = computeClosests(R_O0O1, R_O0O1)
    for (idS,path,compilerOption,name, pathJson) in OAll:
        MD["<>"][idS] = {}
        for (idS2,path2,compilerOption2,name2, pathJson2) in OAll:
            if idS == idS2:
                continue
            if closestDALL[nameDS][idS][1] == idS2:
                d = 0
            else:
                d = 1
            MD["<>"][idS][idS2] = (name,name2,compilerOption,compilerOption2,d, closestDALL[nameDS][idS][2])
