...
from classificationSFCG import transferInstructions, arithmeticIntructions, callIntructions
...
def getfunc_consts(func):
	...
	for bl in blocks:
		...
		strings += [sum([ sum([ord(x) for x in s])  for s in strs])]
		consts += [sum(conts)]
...
def calArithmeticIns(bl):
	...
	calls = {}
	for x in ["addss","addsd","subss","mulsd","mulss","divsd","divss","imul","div","idiv","adc","sbb","inc","dec"]:
		calls[x] = 1
	calls.update(x86_AI)
	calls.update(mips_AI)
	calls.update(arithmeticIntructions())
	start = bl[0]
	end = bl[1]
	invoke_num = 0
	inst_addr = start
...
def calTransferIns(bl):
	...
	calls.update(transferInstructions())
	start = bl[0]
	end = bl[1]
	invoke_num = 0
	inst_addr = start
