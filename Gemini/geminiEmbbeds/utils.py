...
def generate_epoch_pair(Gs, classes, M, output_id = False, load_id = None):
    ...
    if load_id is None:
        ...
        while st < len(Gs):
            if output_id:
                X1, X2, m1, m2, y, X1F, X2F, pos_id, neg_id = get_pair(Gs, classes,
                        M, st=st, output_id=True)
                id_data.append( (pos_id, neg_id) )
            else:
                X1, X2, m1, m2, y, X1F, X2F = get_pair(Gs, classes, M, st=st)
            epoch_data.append( (X1,X2,m1,m2,y,X1F,X2F) )
            st += M
    else:   ## Load from previous id data
        id_data = load_id
        for id_pair in id_data:
            X1, X2, m1, m2, y, X1F, X2F = get_pair(Gs, classes, M, load_id=id_pair)
            epoch_data.append( (X1, X2, m1, m2, y, X1F,X2F) )
...
def get_pair(Gs, classes, M, st = -1, output_id = False, load_id = None):
    ...
    X1_function = [["",0] for i in range(M) ]
    X2_function = [["",0] for i in range(M) ]
    ...
    for i in range(M_pos):
        ...
        g1 = Gs[pos_ids[i][0]]
        g2 = Gs[pos_ids[i][1]]
        X1_function[i][0] = g1.name
        X2_function[i][0] = g2.name
        X1_function[i][1] = g1.label
        X2_function[i][1] = g2.label
    ...
    for i in range(M_pos, M_pos + M_neg):
        ...
        for u in range(g2.node_num):
            ...
            for v in g2.succs[u]:
                node2_mask[i, u, v] = 1
    if output_id:
        return X1_input,X2_input,node1_mask,node2_mask,y_input,X1_function,X2_function,pos_ids,neg_ids
    else:
        return X1_input,X2_input,node1_mask,node2_mask,y_input,X1_function,X2_function
...
def train_epoch(model, graphs, classes, batch_size, load_data=None):
    ...
    for index in perm:
        ...
        X1, X2, mask1, mask2, y, _, _ = cur_data
...
def get_vectors(model, batch_size, data, dictF):
    ...
    for cur_data in tqdm(data):
        ...
        for i in range(len(X1F)):            
            idS = X1F[i][0] #int()      
            if not(idS in vecById):
                vecById[idS] = {}           
            nameF = reverseFID[X1F[i][1]]
            vecById[idS][nameF] = vec[i]
    return vecById