...
from Basic_Ablation_Preprocessing import readTableBasicAblationPreprocessing
from Basic_Ablation_CountCloneSearches import readBasicAblationCountCloneSearches

def readBasicAblationSpeed():
	ABS_PATH = "/".join(os.path.abspath(__file__).split("/")[:-1])

	frameworks = ["simCG", "simCFG", "PSS"]
	...
	tablePreprocessing = readTableBasicAblationPreprocessing()

	for f in frameworksData:
		if f in tablePreprocessing:
			frameworksData[f]["Total"] += tablePreprocessing[f]["Total"] 
		
	tableCSCount = readBasicAblationCountCloneSearches()	
	
	for f in frameworksData:	
		frameworksData[f]["AVG"] = formatTimeAvg(frameworksData[f]["Total"]/tableCSCount[f]["# Clone Searches"])
		if f in tablePreprocessing:
			frameworksData[f]["AVG"] += " ("+formatTimeAvg(tablePreprocessing[f]["Total"]/tableCSCount[f]["# Clone Searches"]) + ")"

	for f in frameworksData:	
		frameworksData[f]["Total"] = formatTime(frameworksData[f]["Total"])
		if f in tablePreprocessing:
